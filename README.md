...
- The static build output is `dist/` (generated by `bun run build`, which runs `vite build` followed by `scripts/prerender.ts`).
- API routes run as a single Netlify Function at `/.netlify/functions/api` (see `netlify/functions/api.ts`).
- All API calls from the client are routed via the redirect in `netlify.toml`: `/api/*` → `/.netlify/functions/api/:splat`.
- Environment variables (Supabase, Stripe, Resend, Telegram, etc.) are configured in the **Netlify dashboard** under Site settings → Environment variables, not in source.
- Variables prefixed with `VITE_` are exposed to client-side code via Vite at build time.

## Deployment

The site is deployed to Netlify. `netlify.toml` defines the build command (`bun install && bun run build`), the publish directory (`dist/`), and a redirect that mounts the Hono API at `/.netlify/functions/api`. Pushes to the main branch trigger an automatic deploy.