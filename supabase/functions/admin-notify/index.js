    ...
    try {
        ...
        switch (type) {
            ...
            case "contact":
                ...
                break;
            default:
                return new Response(JSON.stringify({ error: `Unknown notification type: ${type}` }), { status: 400, headers: { "Content-Type": "application/json" } });
        }
        const delivered = await sendEmail(subject, text);
        return new Response(JSON.stringify({ ok: true, delivered, type }), { headers: { "Content-Type": "application/json" } });
    }
    catch (err) {
