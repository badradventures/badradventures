...
// ---------------------------------------------------------------------------
// Session state (ephemeral — resets on cold start, same as the Bun server)
// ---------------------------------------------------------------------------
...
async function isAllowedChat(chatId: unknown): Promise<boolean> {
  if (typeof chatId !== "number" && typeof chatId !== "string") return false;
  const key = String(chatId);
  if (await isTelegramChatAllowed(key)) return true;
...
function buildRentItemSummary(it: ParsedRent): string {
...
function parseRentText(input: string, imageHint?: string):
  ...
  if (!id) errors.push({ field: "id", message: "Add an id like 4p-tent-ld." });

  const type = (fields["type"] ?? "").toLowerCase();
  if (!["tent", "bnb", "gear"].includes(type)) errors.push({ field: "type", message: "Must be: tent, bnb, gear." });

  const name = fields["name"]?.trim();
  if (!name) errors.push({ field: "name", message: "Add a name." });

  const summary = fields["summary"]?.trim();
  if (!summary) errors.push({ field: "summary", message: "Add a summary." });

  const description = fields["description"]?.trim();
  if (!description) errors.push({ field: "description", message: "Add a description." });

  const location = fields["location"]?.trim();
  if (!location) errors.push({ field: "location", message: "Add a location." });

  const price = Number(fields["pricepernightgbp"] ?? "");
  if (!Number.isFinite(price) || price < 0) errors.push({ field: "pricePerNightGbp", message: "Add a non-negative number." });

  const capacity = Number(fields["capacity"] ?? "");
  if (!Number.isInteger(capacity) || capacity < 1) errors.push({ field: "capacity", message: "Add a positive integer." });

  const totalUnits = Number(fields["totalunits"] ?? fields["stock"] ?? "");
  if (!Number.isInteger(totalUnits) || totalUnits < 0) errors.push({ field: "totalUnits", message: "Add a non-negative integer." });

  const availableUnits = Number(fields["availableunits"] ?? "");
  if (!Number.isInteger(availableUnits) || availableUnits < 0) errors.push({ field: "availableUnits", message: "Add a non-negative integer." });

  const unitLabel = fields["unitlabel"] ?? "per night";
  if (!VALID_UNIT_LABELS.includes(unitLabel as UnitLabel)) errors.push({ field: "unitLabel", message: `Must be: ${VALID_UNIT_LABELS.join(", ")}.` });
...
async function updateHike(id: string, body: Record<string, unknown>): Promise<{ ok: boolean; error?: string }> {
  try {
    const patch: HikePatch = {};
    ...
    if (body.region !== undefined) patch.region = body.region as string;
    if (body.date !== undefined) patch.date = body.date as string;
    if (body.duration !== undefined) patch.duration = body.duration as string;
    if (body.difficulty !== undefined) patch.difficulty = body.difficulty as string;
    if (body.summary !== undefined) patch.summary = body.summary as string;
    ...
    if (body.hero !== undefined) patch.hero = body.hero as string;
    if (body.guide !== undefined) patch.guide = body.guide as string;
    if (body.spotsTotal !== undefined) patch.spotsTotal = Number(body.spotsTotal);
...
async function saveRentItem(it: ParsedRent): Promise<{ ok: true; id: string } | { ok: false; error: string }> {
  try {
    ...
    if (existing) return { ok: false, error: `Item "${it.id}" already exists.` };
...
async function updateRentItem(id: string, body: Record<string, unknown>): Promise<{ ok: boolean; error?: string }> {
  try {
    ...
    if (body.pricePerNightGbp !== undefined) patch.pricePerNightGbp = Number(body.pricePerNightGbp);
    if (body.capacity !== undefined) patch.capacity = Number(body.capacity);
    if (body.totalUnits !== undefined) patch.totalUnits = Number(body.totalUnits);
    if (body.availableUnits !== undefined) patch.availableUnits = Number(body.availableUnits);
...
async function getFileDownloadUrl(fileId: string): Promise<string | null> {
  try {
...
async function handleTextOrPhoto(chat: string, text: string, messageId: number, imageUrl?: string) {
  ...
  if (session) {
    ...
    if (session.kind.type === "rent-new") { await startRentDraft(chat, trimmed, imageUrl, messageId); return; }
    if (session.kind.type === "rent-edit") { await handleRentEditPick(chat, trimmed, messageId); return; }
    if (session.kind.type === "rent-edit-field") { await handleRentEditField(chat, trimmed, messageId); return; }
...
async function startHikeDraft(chat: string, text: string, imageUrl?: string, messageId?: number) {
...
async function handleHikeEditField(chat: string, text: string, messageId: number) {
  ...
  const field = s.kind.field as HikeField;
  if (/^skip$/i.test(trimmed)) { await advanceHikeFieldQueue(chat, s, messageId); return; }
...
async function startRentDraft(chat: string, text: string, imageUrl?: string, messageId?: number) {
...
async function handleRentEditField(chat: string, text: string, messageId: number) {
  ...
  const field = s.kind.field as RentField;
  if (/^skip$/i.test(trimmed)) { await advanceRentFieldQueue(chat, s, messageId); return; }
