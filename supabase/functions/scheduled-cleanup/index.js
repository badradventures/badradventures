...
Deno.serve(async (req) => {
    try {
        ...
        let olderThanDays = parseInt(Deno.env.get("DEFAULT_CLEANUP_DAYS") || "365", 10);
        ...
        console.log(`[scheduled-cleanup] deleting contact_messages older than ${olderThanDays} days (before ${new Date(cutoff).toISOString()})`);
        ...
        const deleted = Array.isArray(data) ? data.length : data?.count ?? 0;
        console.log(`[scheduled-cleanup] deleted ${deleted} messages`);
        return new Response(JSON.stringify({ ok: true, deletedMessages: deleted, olderThanDays }), {
            headers: { "Content-Type": "application/json" },
        });
    }
    catch (err) {
