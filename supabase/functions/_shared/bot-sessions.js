...
export async function saveSession(chatId, session) {
    if (session === null) {
        const { error } = await supabaseAdmin()
            ...
            .eq("chat_id", chatId);
        if (error)
            console.error("[bot-sessions] delete error:", error.message);
        return;
    }
    const { error } = await supabaseAdmin()
        .from("bot_sessions")
        .upsert({ chat_id: chatId, session_data: JSON.stringify(session), updated_at: new Date().toISOString() }, { onConflict: "chat_id" });
    if (error)
        console.error("[bot-sessions] upsert error:", error.message);
}
