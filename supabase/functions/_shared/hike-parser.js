...
// Ported from backend-lib/telegram-hike-parser.ts for Deno.
const KNOWN_DIFFICULTIES = ["Easy", "Moderate", "Challenging", "Strenuous"];
const DIFF_PATTERN = new RegExp(`\\b(${KNOWN_DIFFICULTIES.join("|")}|easy|moderate|challenging|strenuous|hard|difficult|beginner|family[-\\s]?friendly)\\b`, "i");
const PRICE_PATTERN = /(?:£\s*)?(\d{1,4}(?:\.\d{1,2})?)/i;
const DATE_PATTERN = /\b(\d{4}-\d{2}-\d{2})\b|(\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4})\b/i;
...
const GUIDE_PATTERN = /(?:led by|lead(?:ing)?(?:\s+guide)?|guide(?:\s+is)?|with guide)\s+([A-Z][a-zA-Z][\w\s'-]{1,30})/i;
const KNOWN_REGIONS = [
    "Peak District", "Lake District", "Yorkshire Dales", "Snowdonia",
    "Brecon Beacons", "Scottish Highlands", "Dartmoor", "Pembrokeshire",
    "Cornwall", "Norfolk", "Suffolk", "New Forest", "South Downs",
    "Cotswolds", "Isle of Skye", "Cairngorms", "Pitons", "Hadrian's Wall",
];
...
function normaliseDate(raw) {
    ...
    if (Number(y) < 2000 || Number(y) > 2100)
        return null;
...
function normaliseDifficulty(raw) {
    ...
    if (r === "moderate" || r === "medium")
        return "Moderate";
    if (r === "challenging" || r === "hard" || r === "difficult")
        return "Challenging";
    if (r === "strenuous" || r === "tough" || r === "extreme")
        return "Strenuous";
    return null;
}
function inferDifficultyFromText(text) {
    const lower = text.toLowerCase();
    if (/\b(strenuous|extreme|tough|very hard|epic)\b/.test(lower))
        return "Strenuous";
    if (/\b(challenging|hard|difficult|advanced)\b/.test(lower))
        return "Challenging";
    if (/\b(moderate|medium|intermediate)\b/.test(lower))
        return "Moderate";
    if (/\b(easy|beginner|gentle|family|introduction)\b/.test(lower))
        return "Easy";
    return null;
}
...
function extractTags(text) {
    ...
    const lower = text.toLowerCase();
    const candidates = [
        [/\bfamily[- ]?friendly\b/, "Family friendly"],
        [/\bweekend\b/, "Weekend"],
        [/\bcamping\b|\bcamp\b|\bwild[- ]?camp\b/, "Camping"],
        [/\bsummit\b/, "Summit"],
        [/\bwater\b|\bkayak\b|\bcoast\b/, "Water sports"],
        [/\bbeginner\b/, "Beginner"],
        [/\bmulti[- ]?day\b/, "Multi-day"],
        [/\bday[- ]?hike\b/, "Day hike"],
    ];
...
function deriveTitle(text, region) {
    ...
    return region ? `${titleCase(region)} Hike` : "New Hike";
}
function deriveId(title, date) {
...
export function parseHikeText(input, options) {
    const errors = [];
    ...
    if (dateMatch) {
        ...
        errors.push({ field: "date", message: "No date found. Add a date like 2026-08-12." });
    }
    // Price
    let priceGbp = 0;
    ...
    if (!difficulty) {
        errors.push({ field: "difficulty", message: `Couldn't tell difficulty. Use one of: ${KNOWN_DIFFICULTIES.join(", ")}.` });
    }
    // Duration
    let duration = "Full day";
    const durMatch = text.match(DURATION_PATTERN);
    if (durMatch)
        duration = titleCase(durMatch[1].replace(/\s+/g, " ").trim());
    // Region
    let region = inferRegion(text);
    if (!region) {
        const m = text.match(/\b(?:in|at|to)\s+([A-Z][\w\s'-]{2,30}?)(?:\.|,|\n| on\b)/);
        if (m)
            region = titleCase(m[1].trim());
    }
    if (!region) {
        errors.push({ field: "region", message: `Couldn't tell the region. Mention one of: ${KNOWN_REGIONS.join(", ")}.` });
    }
    // Description
    let description = "";
    ...
    if (!description) {
        errors.push({ field: "description", message: 'No description found. Add a <code>Description:</code> section.' });
    }
    // Guide
    let guide = "Abu Jabal";
    ...
    if (!location) {
        ...
        if (m)
            location = titleCase(m[1]);
    }
    const summary = description
        ? description.split(/[.!?]\s*/)[0]?.trim() + "."
        : `${title} — new Badr Adventures trip.`;
