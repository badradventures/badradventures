...
export async function syncHikeToStripe(hike: HikeRow): Promise<{
  productId?: string;
  priceId?: string;
}> {
...
export async function removeHikeFromStripe(
...
export async function syncEquipmentToStripe(item: EquipmentRow): Promise<{
  productId?: string;
  priceId?: string;
}> {
  if (!isStripeConfigured()) return {};
...
export async function removeEquipmentFromStripe(
  productId?: string | null,
  equipmentId?: string,
): Promise<boolean> {
  if (!isStripeConfigured()) return false;
