...
function normaliseDifficulty(raw: string): Difficulty | null {
  ...
  if (r === "easy" || r === "beginner" || r === "family-friendly" || r === "familyfriendly") return "Easy";
...
export function parseHikeText(
  ...
  if (dateMatch) {
    ...
    if (!date) errors.push({ field: "date", message: `Couldn't parse date "${dateMatch[0]}". Use YYYY-MM-DD.` });
  } else {
  ...
  if (priceMatch) {
    ...
    if (!Number.isNaN(n) && n >= 0 && n < 100000) priceGbp = Math.round(n * 100) / 100;
  }
  ...
  if (diffMatch) difficulty = normaliseDifficulty(diffMatch[1]);
  if (!difficulty) difficulty = inferDifficultyFromText(text);
  if (!difficulty) {
  ...
  else if (options?.imageHint) image = options.imageHint;
